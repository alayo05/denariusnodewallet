...
exports.address = function (req, res) {
    ...
    client.getNewAddress(`dnrw(${username})`, function (error, address, resHeaders) {
        ...
        res.render('account/newaddress', { title: 'New REEX Address', user: req.user, address: address, data_url: data_url });
    });
  });
};
...
exports.withdraw = (req, res, next) => {
    ...
    var valid = WAValidator.validate(`${sendtoaddress}`, 'REEX');

    if (parseFloat(amount) - fee > balance) {

        req.flash('errors', { msg: 'Withdrawal amount exceeds your REEX balance'});
    ...
    if (valid) {
        ...
        client.sendFrom(`dnrw(${username})`, `${sendtoaddress}`, parseFloat(`${amount}`), 10, function (error, sendFromtx, resHeaders) {
            if (error) {
                ...
                req.flash('success', { msg: `Your ${vamount} REEX was sent successfully! TX ID: ${sendtx}` });
                return res.redirect('/withdraw');
            }
        });
        ...
        req.flash('errors', { msg: 'You entered an invalid ReeCore (REEX) Address!' });
        return res.redirect('/withdraw');
    }
  }
  });
